...
from moveit_msgs.msg import (AttachedCollisionObject, CollisionObject,
                             DisplayTrajectory, ObjectColor, PlanningScene,
                             RobotTrajectory)
...
from move_group_utils.srv import (ClearMarkerArray, VisualizePoses,
                                  VisualizeTrajectory)
from pilz_robot_program.pilz_robot_program import (Circ, Lin,
                                                   MoveGroupSequence, Ptp,
                                                   Sequence,
...
class MoveGroupUtils:
    ...
    def plan_and_execute(self, sequence: Sequence, max_iterations: int = 10
                         ) -> bool:

        success = False
        while not success and max_iterations > 0:
            try:
                success, plan = self.sequencer.plan(sequence)[:2]
            except SequencePlanningError:
                pass
            max_iterations -= 1
            rospy.loginfo(f'planning iteration {10 - max_iterations}')
        if not success:
            rospy.logerr('Failed to plan sequence')
            return False
        publish_trajectory_markers(plan[0])
        self.display_trajectory(plan)
        self.sequencer.execute(plan)
        return True

    def plan(self, sequence: Sequence, max_iterations: int = 10
             ) -> Union[bool, Tuple[bool, RobotTrajectory]]:
    ...
    def set_color(self, name: str, r: float, g: float, b: float, a: float
                  ) -> None:
        color = ObjectColor()
        color.id = name
        color.color.r = r
    ...
    def add_ground_cube(self) -> bool:
        ...
        self.scene.add_box('ground_cube', pose, (2, 2, 1))

        return self.wait_for_state_update('ground_cube', object_is_known=True)

    def setup_scene(self) -> bool:
        self.add_ground_cube()
        self.set_color('ground_cube', 0.8, 0.8, 0.8, 0.5)
        self.send_colors()
        return True
